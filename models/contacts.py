...
class Contacts(Model):
    ...
    def update(id):
        ...
        updateContact['userImg'] = Model.validationExists("userImg", userImg, contact_data)
        
        name = input(f"Enter your name (default {contact_data['name']}): ")
        updateContact['name'] = Model.validationExists("name", name, contact_data)
        
        surname = input(f"Enter your surname (default {contact_data['surname']}): ")
        updateContact['surname'] = Model.validationExists("surname", surname, contact_data)
        
        email = input(f"Enter your email (default {contact_data['email']}): ")
        updateContact['email'] = Model.validationExists("email", email, contact_data)
        
        phone = input(f"Enter your phone (default {contact_data['phone']}): ")
        updateContact['phone'] = Model.validationExists("phone", phone, contact_data)
        
        date = input(f"Enter date when wrote the review (YYYY-MM-DD) (default {contact_data['date']}): ")
        updateContact['date'] = Model.validationExists("date", date, contact_data)
        
        subject = input(f"Enter the subject of review (default {contact_data['subject']}): ")
        updateContact['subject'] = Model.validationExists("subject", subject, contact_data)
        
        message = input(f"Enter your message (default {contact_data['message']}): ")
        updateContact['message'] = Model.validationExists("message", message, contact_data)
