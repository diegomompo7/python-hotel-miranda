...
class Contacts(Model):
    ...
    def create():
        ...
        if stars <= 1 or 5 <= stars:
            return Model.validationPositive("stars", messageStars, None)
        
        isArchived = False

        Model.contact(userImg, name, surname, email, phone, date, subject, message, stars, isArchived, None)

    def update(id):
        contact_data = Contacts.view(str(id))

        userImg = input(
            f"Enter your image (default {contact_data.get('userImg')}): "
        )
        name = input(
            f"Enter your name (default {contact_data.get('name')}): "
        )
        surname = input(
            f"Enter your surname (default {contact_data.get('surname')}): "
        )
        email = input(
            f"Enter your email (default {contact_data.get('email')}): "
        )
        phone = input(
            f"Enter your phone (default {contact_data.get('phone')}): "
        )
        date = input(
        ...
        if stars <= 1 or 5 <= stars:
            print("w")
            return Model.validationPositive("stars", messageStars, contact_data)
        
        isArchived = input(
            f"Enter if you want archived the review (default {contact_data.get('is_archived')}): "
        )

        Model.contact(userImg, name, surname, email, phone, date, subject, message, stars, isArchived, contact_data)
