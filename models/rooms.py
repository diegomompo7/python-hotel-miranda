...
class Rooms(Model):
    ...
    def create():
        ...
        if offer == "YES":
            ...
            discount = Model.validationPositive('discount', messageDiscount, None)
        else:
            discount = 0
    ...
    def update(id):
        ...
        amenitySelect = executeQuery("SELECT amenity, room_id FROM amenities WHERE room_id=%s", id, "GET")
        amenities_data = []
        ...
        roomTypeMessage = (f"Insert a room type {type} (default {room_data['roomType']}): ")
        ...
        offerMessage = f"Insert if there's offer or not {isOffer} (default {room_data['offer']}): "
        ...
        statusMessage = (f"Insert a status {roomStatus} (default {room_data['status']}): ")
        ...
        for amenity in amenities_data:
            if amenity not in amenities:
                executeQuery("DELETE FROM amenities WHERE amenity=%s and room_id=%s", (f"\'{amenity}\'", id), "DELETE")     