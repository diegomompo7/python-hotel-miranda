...
class Bookings(Model):
    ...
    def create():
        ...
        checkSpecialRequest = Model.validationExists("specialRequest", specialRequest, None)
    ...
    def update(id):
        ...
        updateBook["name"] = Model.validationExists("name", name, booking_data)

        check_in = input(f"Date to entry (YYYY-MM-DD) (default {updateBook['check_in']}): ")
        updateBook["check_in"] = Model.validationExists("check_in", check_in, booking_data)
        
        hour_in = input(f"Date to entry (HH:MM)  (default {updateBook['hour_in']}): ")
        updateBook["hour_in"] = Model.validationExists("hour_in", hour_in, booking_data)
        
        check_out = input(f"Date to entry (YYYY-MM-DD)  (default {updateBook['check_out']}): ")
        updateBook["check_out"] = Model.validationExists("check_out", check_out, booking_data)
        
        hour_out = input(f"Date to entry (HH:MM) (default {updateBook['hour_out']}): ")
        updateBook["hour_out"] = Model.validationExists("hour_out", hour_out, booking_data)
        ...
        updateBook['specialRequest'] = Model.validationExists("specialRequest", specialRequest, booking_data)
